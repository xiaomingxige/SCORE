...
class YOLOLoss(nn.Module):    
    ...
    def get_assignments(self, num_gt, total_num_anchors, gt_bboxes_per_image, gt_classes, bboxes_preds_per_image, cls_preds_per_image, obj_preds_per_image, expanded_strides, x_shifts, y_shifts):
        ...
        # print(pair_wise_cls_loss)
        # print(pair_wise_ious_loss)
        # print(100000.0 * (~is_in_boxes_and_center).float())
    ...
    def dynamic_k_matching(self, cost, pair_wise_ious, gt_classes, num_gt, fg_mask):
        ...
        for gt_idx in range(num_gt):
            ...
            #   给每个真实框选取最小的动态k个点
            #------------------------------------------------------------#
            # print(cost, cost[gt_idx].shape, dynamic_ks[gt_idx].item(), num_gt)
            _, pos_idx = torch.topk(cost[gt_idx], k=dynamic_ks[gt_idx].item(), largest=False)

            # import numpy as np
            # pos_idx = torch.from_numpy(np.array([0])).cuda(0)
            # print(pos_idx)
